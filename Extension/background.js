...
function setGlobalState(e, t) {
  if (setToolbarIcon((g_theState = e) === ContextState.SessionKeySet ? "PasswordsToolbar" : "PasswordsToolbarUnpaired"), !t) try {
    g_portToPopup && g_portToPopup.postMessage({
...
function sendMessageToPopupAndCompletionList(e) {
  try {
    g_portToPopup && g_portToPopup.postMessage(e)
  } catch (e) { }
  try {
      ...
      if (o.hostname, t.tab.id, t.frameId, e.from, "content" === e.from) switch (e.subject, e.subject) {
        ...
        case "CmdAddSetUpTOTPContextMenu":
          ...
          chrome.contextMenus.create({
            title: a ? `${g_localizer.getMessage("divTOTPMenu")} (${e.totpSetupURL})` : g_localizer.getMessage("divTOTPMenu"),
            id: e.totpSetupURL,
            contexts: ["all"]
...
function scanForOTPURI() {
  return (g_nativeAppCapabilities || DefaultCapabilities).scanForOTPURI
}
...
chrome.tabs.onUpdated.addListener(((e, t, s) => {
  ...
  if (JSON.stringify(e), chrome.contextMenus.removeAll(), !g_nativeAppPort) return;
  let t = {
    cmd: 8,
    tabId: e.tabId,
    event: 1
